...
fourOutputs _scaled_dot_product_efficient_attention_batch_rule(
  ...
  if (attn_bias.has_value() && attn_bias->defined()) {
    if (attn_bias_bdim.has_value()) {
      ...
      auto tmp = ensure_has_bdim(attn_bias.value(), false, batch_size);
      attn_bias_ = tmp.flatten(0, 1);
    }
  }
...
fourOutputs _scaled_dot_product_efficient_attention_backward_batch_rule(
  ...
  grad_out = ensure_has_bdim(grad_out, grad_out_bdim.has_value(), batch_size);
  query_ = ensure_has_bdim(query_, query_bdim.has_value(), batch_size);
  key_ = ensure_has_bdim(key_, key_bdim.has_value(), batch_size);
  value_ = ensure_has_bdim(value_, value_bdim.has_value(), batch_size);
  out_ = ensure_has_bdim(out_, out_bdim.has_value(), batch_size);
  logsumexp_ = ensure_has_bdim(logsumexp_, logsumexp_bdim.has_value(), batch_size);

  grad_out = grad_out.flatten(0, 1);
  query_ = query_.flatten(0, 1);
  key_ = key_.flatten(0, 1);
  value_ = value_.flatten(0, 1);
  out_ = out_.flatten(0, 1);
  logsumexp_ = logsumexp_.flatten(0, 1);
  ...
  if (attn_bias.defined()) {
    if (attn_bias_bdim.has_value()) {
      ...
      attn_bias_ = ensure_has_bdim(attn_bias, false, batch_size);
      attn_bias_ = attn_bias_.flatten(0, 1);
    }
  }
  ...
  // Outputs can be undefined when grad_input_mask[i] is false
...
threeOutputs _scaled_dot_product_flash_attention_backward_batch_rule(
  ...
  grad_out_ = grad_out_.flatten(0, 1);
  query_ = query_.flatten(0, 1);
  key_ = key_.flatten(0, 1);
  value_ = value_.flatten(0, 1);
  out_ = out_.flatten(0, 1);
